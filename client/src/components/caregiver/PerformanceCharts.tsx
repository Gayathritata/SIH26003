        ...
        <div style={{ width: '100%', overflowX: 'auto' }}>
          <svg viewBox={`0 0 ${width} ${height}`} style={{ width: '100%', height: 'auto', minWidth: '320px' }}>
            ...
            {[0, 25, 50, 75, 100].map((val) => {
              ...
              return (
                <g key={val}>
                  ...
                  <text x={padding - 8} y={y + 4} fill="rgba(255,255,255,0.4)" fontSize="10" textAnchor="end">{val}%</text>
                </g>
              );
            })}
            ...
            {sorted.map((s, idx) => {
              ...
              const y = height - padding - (Math.min(100, s.accuracy) / 100) * (height - padding * 2);
              return (
                <circle key={idx} cx={x} cy={y} r="5" fill="#10B981" stroke="#FFFFFF" strokeWidth="2" />
              );
            })}
          </svg>
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))', gap: '20px' }}>
        ...
        <div className="glass-panel" style={{ padding: '24px', display: 'flex', flexDirection: 'column', gap: '14px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            <span style={{ fontSize: '14px', fontWeight: '800', color: '#F472B6' }}>
          ...
          <div style={{ width: '100%', overflowX: 'auto' }}>
            <svg viewBox={`0 0 ${width} ${height}`} style={{ width: '100%', height: 'auto', minWidth: '300px' }}>
              <polyline fill="none" stroke="#EC4899" strokeWidth="3" points={pointsScore} strokeLinecap="round" strokeLinejoin="round" />
              {sorted.map((s, idx) => {
                const x = padding + (idx / Math.max(1, sorted.length - 1)) * (width - padding * 2);
                const y = height - padding - ((s.score || 0) / maxScore) * (height - padding * 2);
                return <circle key={idx} cx={x} cy={y} r="4" fill="#EC4899" stroke="#FFFFFF" strokeWidth="2" />;
              })}
            </svg>
          </div>
        </div>
        ...
        <div className="glass-panel" style={{ padding: '24px', display: 'flex', flexDirection: 'column', gap: '14px' }}>
          ...
          <div style={{ width: '100%', overflowX: 'auto' }}>
            <svg viewBox={`0 0 ${width} ${height}`} style={{ width: '100%', height: 'auto', minWidth: '300px' }}>
              <polyline fill="none" stroke="#3B82F6" strokeWidth="3" points={pointsTime} strokeLinecap="round" strokeLinejoin="round" />
              {sorted.map((s, idx) => {
                const x = padding + (idx / Math.max(1, sorted.length - 1)) * (width - padding * 2);
                const y = height - padding - (Math.min(maxTime, s.completionTime || 0) / maxTime) * (height - padding * 2);
                return <circle key={idx} cx={x} cy={y} r="4" fill="#3B82F6" stroke="#FFFFFF" strokeWidth="2" />;
              })}
            </svg>
          </div>
        </div>
