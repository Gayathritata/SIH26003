...
import { Brain, LogIn, Key, Mail, UserCheck, AlertCircle, ArrowRight, ShieldCheck } from 'lucide-react';
...
export const LoginScreen: React.FC<Props> = ({ onSuccess, onNavigateRegister, onNavigateForgot, lang }) => {
  ...
  const handleQuickDemoLogin = async (role: 'elderly' | 'caregiver' | 'admin') => {
    setLoading(true);
    setError(null);
    try {
      const demoEmail = role === 'caregiver' ? 'caregiver@demo.mindmate' : (role === 'admin' ? 'admin@demo.mindmate' : 'asha.devi@demo.mindmate');
      const res = await authService.login(demoEmail, 'demo123456');
      onSuccess(res.user);
    } catch (err: any) {
    ...
    >
      ...
      >
        ...
        <div style={{ textAlign: 'center', marginBottom: '28px' }}>
          ...
          <p style={{ fontSize: '15px', color: '#94A3B8', marginTop: '4px' }}>Firebase Secure Authentication</p>
        </div>
        ...
        <form onSubmit={handleLogin} style={{ display: 'flex', flexDirection: 'column', gap: '18px' }}>
          ...
          <button type="submit" className="btn-primary btn-emerald" disabled={loading} style={{ minHeight: '56px', marginTop: '6px' }}>
            <LogIn size={22} /> {loading ? 'Authenticating...' : 'Sign In with Firebase'}
          </button>
        </form>

        {/* Demo Quick Logins Box for Hackathon Evaluation */}
