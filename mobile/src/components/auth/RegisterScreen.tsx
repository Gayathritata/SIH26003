...
import { User, Mail, Key, UserCheck, AlertCircle, ArrowLeft, ShieldCheck } from 'lucide-react';
...
export const RegisterScreen: React.FC<Props> = ({ onSuccess, onNavigateLogin, lang }) => {
  ...
  const [role, setRole] = useState<'elderly' | 'caregiver' | 'admin'>('elderly');
      ...
      >
        ...
        <div style={{ textAlign: 'center', marginBottom: '24px' }}>
          ...
          <p style={{ fontSize: '15px', color: '#94A3B8', marginTop: '4px' }}>
            Firebase Auth Registration & MongoDB Sync
          </p>
        </div>
        ...
        <form onSubmit={handleRegister} style={{ display: 'flex', flexDirection: 'column', gap: '18px' }}>
          ...
          <button type="submit" className="btn-primary btn-emerald" disabled={loading} style={{ minHeight: '56px', marginTop: '10px' }}>
            <UserCheck size={22} /> {loading ? 'Registering...' : 'Register with Firebase Auth'}
          </button>
        </form>
